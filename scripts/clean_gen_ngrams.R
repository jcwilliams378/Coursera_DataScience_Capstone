...
if (!file.exists("./final/en_US/train.unk.txt")) {
        stop("error: please make sure dir has train.unk.txt")
}

# convert text (char vec) to corpus
library(tm)
if (!exists("./final/en_US/train.unk")) {
        train.unk <- readLines("./final/en_US/train.unk.txt")
}
...
# make bigram csv
if (!file.exists("./final/en_US/n2.csv")) {
        ...
        write.csv(n2, "./final/en_US/n2.csv", row.names=FALSE)
        rm(n2)  # re-import later
        print("wrote n2 csv")
}

# make trigram csv
if (!file.exists("./final/en_US/n3.csv")) {
        ...
        write.csv(n3, "./final/en_US/n3.csv", row.names=FALSE)
        rm(n3)  # re-import later
        print("wrote n3 csv")
}

# make quadgram csv
if (!file.exists("./final/en_US/n4.csv")) {
        ...
        write.csv(n4, "./final/en_US/n4.csv", row.names=FALSE)
        rm(n4)  # re-import later
        print("wrote n4 csv")
}

# make Quintgram csv
if (!file.exists("./final/en_US/n5.csv")) {
        ...
        write.csv(n5k, "./final/en_US/n5.csv", row.names=FALSE)
